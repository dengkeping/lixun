...
impl ThemeResolver {
    ...
    /// Return the matugen-generated colors file path
    /// (`${config_dir}/lixun/colors.css`) regardless of whether the
    /// file exists. The path is the default sink for matugen's
    /// `[templates.lixun]` block; the actual location may be
    /// overridden by `matugen_colors_path` under `[gui]` in
    /// `config.toml`, in which case callers pass that path to the
    /// watcher instead of this default. Existence is not checked
    /// here so the watcher can detect file creation on the user's
    /// first matugen run.
    #[allow(dead_code)]
    pub fn colors_css_path(&self) -> PathBuf {
        self.config_dir.join("lixun").join("colors.css")
    }
...
mod tests {
    ...
    #[test]
    fn colors_css_path_returns_path_regardless_of_existence() {
        let tmp = TempDir::new().unwrap();
        let resolver = ThemeResolver::new(tmp.path().to_path_buf());
        let expected = tmp.path().join("lixun").join("colors.css");
        assert_eq!(resolver.colors_css_path(), expected);
        // Same answer after the file is created, so the watcher can
        // be primed with the path before matugen first writes it.
        fs::create_dir_all(tmp.path().join("lixun")).unwrap();
        fs::write(&expected, "/* generated by matugen */").unwrap();
        assert_eq!(resolver.colors_css_path(), expected);
    }
