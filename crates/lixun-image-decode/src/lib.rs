...
pub fn prepare_batch(paths: &[PathBuf]) -> Result<(Vec<PathBuf>, Vec<TempFile>)> {
    ...
    for path in paths {
        ...
        match ext.as_str() {
            #[cfg(feature = "heic")]
            "heic" | "heif" => match decode_heic_to_temp(path) {
                Ok(temp) => {
            ...
            #[cfg(feature = "jxl")]
            "jxl" => match decode_jxl_to_temp(path) {
                Ok(temp) => {
                    out_paths.push(temp.path().to_owned());
                    temp_files.push(temp);
                }
            ...
            | "rw2" | "pef" => match decode_raw_to_temp(path) {
                Ok(temp) => {
                    out_paths.push(temp.path().to_owned());
                    temp_files.push(temp);
                }
    ...
    if skipped > 0 {
        tracing::info!(
            "Image decode: skipped {} unsupported/corrupt files",
            skipped
        );
    }
...
fn decode_jxl_to_temp(jxl_path: &Path) -> Result<TempFile> {
    ...
    let file = std::fs::File::open(jxl_path).context("Failed to open JXL")?;

    let decoder = JxlDecoder::new(file).context("Failed to create JXL decoder")?;

    let img = DynamicImage::from_decoder(decoder).context("Failed to decode JXL")?;
...
fn decode_raw_to_temp(raw_path: &Path) -> Result<TempFile> {
    ...
    let _raw_image = rawler::decode_file(raw_path).context("Failed to decode RAW")?;

    let srgb = imagepipe::simple_decode_8bit(raw_path, 0, 0)
        .map_err(|e| anyhow::anyhow!("Failed to process RAW: {}", e))?;

    let rgb = RgbImage::from_raw(srgb.width as u32, srgb.height as u32, srgb.data)
...
fn decode_jxl_direct(jxl_path: &Path) -> Result<image::DynamicImage> {
    ...
    DynamicImage::from_decoder(decoder).context("Failed to decode JXL")
}
