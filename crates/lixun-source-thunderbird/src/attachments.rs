...
mod tests {
    ...
    fn test_thunderbird_attachments_source_integration() {
        // index_all() writes extraction results into the XDG cache root, so
        // this must hold HOME_LOCK: a parallel test that has redirected
        // XDG_CACHE_HOME would otherwise receive our cache entries.
